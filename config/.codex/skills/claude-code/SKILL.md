...
- `-c` / `--resume <id>` — continue a previous run. With `--output-format json` you get the `session_id` back, so a follow-up question can reuse the context instead of paying for it again.
